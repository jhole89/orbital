...
// The ReferenceDataFunc type is an adapter to allow the use of ordinary
// function as ReferenceData mutator.
type ReferenceDataFunc func(context.Context, *ent.ReferenceDataMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ReferenceDataFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ReferenceDataMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ReferenceDataMutation", m)
	}
	return f(ctx, mv)
}
